...
import { GetIgredientsById } from '@/hooks/getIngredientbyUd'
...
export default function IngredientPage({ params }: { params: { id: string } }) {
  ...
  const ingredient: Ingredient | undefined = GetIgredientsById(params.id as string)
        ...
        <div className="grid gap-6">
          ...
          <div>
            ...
            {ingredient.substitutes.map((substitute) =>(
                <Badge variant="outline">{substitute.name}</Badge>
              ))}
            </div>
          </div>
          <div>
            ...
            {ingredient.recipes.map((recipe) =>(
               <Link  href={'/recipes/' + recipe.id} className="text-primary hover:underline" prefetch={false}>
               {recipe.name}
             </Link>
              ))}
